...
/**
 * SuperChase Enhanced Multi-Agent Coordinator
...
class SuperChaseCoordinator {
  ...
  async generateEmailDraft(task) {
    ...
    // For now, generating structured template
  ...
  generateEmailSubject(goal) {
    // Smart subject generation based on goal
  ...
  analyzeTask(description) {
    console.log(`\n🔍 DEBUG: Analyzing task: "${description}"`);  // Debug log
    ...
    console.log(`📊 Keyword scores:`, scores);  // Debug log
    
    const primaryAgent = Object.keys(scores).reduce((a, b) => scores[a] > scores[b] ? a : b);
    console.log(`🎯 Primary agent from scores: ${primaryAgent}`);  // Debug log
    ...
    console.log(`🔍 Email checks: email=${emailTask}, message=${messageTask}, send=${sendTask}, contact=${contactTask}, calendar=${calendarTask}, followUp=${followUpSpaced}, follow-up=${followUpHyphen}, draft=${draftTask}`);  // Debug log
    
    if (emailTask || messageTask || sendTask || contactTask || calendarTask || followUpSpaced || followUpHyphen || draftTask) {
      console.log(`✅ OVERRIDE: Routing to Claude for email/calendar task`);  // Debug log
      return {
        primaryAgent: 'claude',
        agents: ['claude'],
        confidence: 0.9,
    ...
    console.log(`⚠️ No override, using primary agent: ${primaryAgent}`);  // Debug log
...
`;
  ...
  start(port = 3000) {
    this.app.listen(port, () => {
      ...
      console.log(`Intake endpoint: http://localhost:${port}/api/intake`);
...
Usage:
  node enhanced-coordinator.js --server [--port 3000]
  node enhanced-coordinator.js route "task description" [priority]
  node enhanced-coordinator.js status
  node enhanced-coordinator.js outputs
  
Server Mode:
  --server, -s     Start HTTP server for intake API
  --port           Specify port (default: 3000)
  
Examples:
  node enhanced-coordinator.js --server
  node enhanced-coordinator.js route "Follow up with client about proposal" high
  node enhanced-coordinator.js status
`);
    return;
  }
